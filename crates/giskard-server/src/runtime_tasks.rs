...
impl RunningTaskState {
        ...
        match event {
            ...
            AgentEvent::TurnCompleted { thread, turn, .. } => {
                ...
                self.tasks.retain(|_, task| {
                    let drop = task.thread_id == *thread
                        && task.turn_id == *turn
    ...
    pub fn set_terminating_by_process(
        &mut self,
        thread_id: ThreadId,
        process_id: &str,
        terminating: bool,
        ...
        let mut changed = false;
        for cmd in self.tasks.values_mut() {
            if cmd.thread_id == thread_id
                && cmd.process_id.as_deref() == Some(process_id)
    ...
    pub fn get_by_process(&self, thread_id: ThreadId, process_id: &str) -> Option<RunningTask> {
        self.tasks
            .values()
            .find(|cmd| cmd.thread_id == thread_id && cmd.process_id.as_deref() == Some(process_id))
            .cloned()
    }
    ...
    pub fn remove_by_process(&mut self, thread_id: ThreadId, process_id: &str) -> bool {
    ...
    pub fn has_running_for_turn(
        &self,
        thread_id: ThreadId,
        turn_id: giskard_core::ids::TurnId,
    ) -> bool {
        self.tasks.values().any(|cmd| {
    ...
    pub fn has_running_for_thread(&self, thread_id: ThreadId) -> bool {
        self.tasks
            .values()
            .any(|cmd| cmd.thread_id == thread_id && command_status_is_running(&cmd.status))
    }
}
...
mod tests {
    ...
    async fn tool_call_is_tracked_like_a_command_and_removed_on_completion() {
        ...
        assert!(store.apply_event(&tool_start(thread, turn, item_id, "search")));
    ...
    async fn running_tool_is_dropped_when_its_turn_ends() {
        ...
        store.apply_event(&tool_start(thread, turn, tool_item, "search"));
        ...
        assert!(store.apply_event(&AgentEvent::TurnCompleted {
            thread,
            turn,
    ...
    async fn running_command_snapshot_tracks_output_and_after_turn() {
        ...
        assert!(store.apply_event(&AgentEvent::ItemStarted {
            thread,
            turn,
            item: ItemStart {
                id: item_id,
    ...
    async fn terminal_item_start_is_not_tracked() {
        ...
        assert!(!store.apply_event(&command_start(
            thread,
            turn,
            ItemId::new(),
            "proc_1",
    ...
    async fn in_progress_completed_item_stays_visible() {
        ...
        assert!(store.apply_event(&AgentEvent::ItemCompleted {
            thread,
            turn,
            item: Item {
                id: item_id,
    ...
    async fn terminal_completed_item_removes_command() {
        ...
        store.apply_event(&AgentEvent::ItemStarted {
            thread,
            turn,
            item: ItemStart {
                id: item_id,
    ...
    async fn running_tasks_with_same_item_id_in_different_turns_are_tracked_separately() {
        ...
        let tasks = store.snapshot(thread);
        assert_eq!(
            tasks.len(),
            2,
    ...
    async fn terminal_failed_and_declined_items_remove_commands() {
        for status in ["failed", "declined"] {
            ...
            assert!(
                store.snapshot(thread).is_empty(),
    ...
    async fn remove_by_process_clears_terminated_command() {
        ...
        assert!(store.remove_by_process(thread, "proc_1"));
        assert!(store.snapshot(thread).is_empty());
        assert!(!store.remove_by_process(thread, "proc_1"));
    }
    ...
    async fn remove_by_process_preserves_unrelated_commands() {
        ...
        store.apply_event(&command_start(thread, turn, first, "proc_1", "in_progress"));
        store.apply_event(&command_start(
            thread,
            turn,
            second,
    ...
    async fn set_terminating_by_process_updates_matching_command() {
        ...
        assert!(!store.set_terminating_by_process(thread, "proc_1", true));
        assert!(store.set_terminating_by_process(thread, "proc_1", false));
        let snapshot = store.snapshot(thread);
        assert_eq!(snapshot.len(), 1);
        assert!(!snapshot[0].terminating);
    }
    ...
    async fn has_running_for_turn_only_matches_active_commands_for_that_turn() {
        ...
        assert!(store.has_running_for_turn(thread, first_turn));
        assert!(store.has_running_for_turn(thread, second_turn));

        store.apply_event(&command_completed(
            thread,
            first_turn,
            first,
            "proc_1",
        ...
        assert!(!store.has_running_for_turn(thread, first_turn));
        assert!(store.has_running_for_turn(thread, second_turn));
        assert!(!store.has_running_for_turn(ThreadId::new(), second_turn));
    }
