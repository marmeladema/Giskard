...
fn browser_exposes_bounded_turn_persistence_recovery() {
    ...
    assert!(body.contains("renderPersistenceRecovery(state.threadId, currentTurnState);"));
    assert!(body.contains("renderPersistenceRecovery(threadId, runtime.turn_state);"));
}
...
fn browser_uses_one_reconciliation_path_for_full_and_incremental_bootstrap() {
    ...
    assert!(page.contains("const pending = state.paginationRequest;"));
    assert!(page.contains("pending.before !== before"));
