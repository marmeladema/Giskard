...
function openDraftThread(pid) {
  ...
  state.loadingHistory = false; state.paginationRequest = null; state.autoFilledTurns = 0;
...
function runtimeSummaryActivity(summary, unread) {
  ...
  return {
    kind: approval ? "approval_requested" : server ? "server_request_received" :
      blocked ? "error" : turnState === "active" ? "progress" : "idle",
...
function applyRuntimeOverview(msg) {
  ...
  for (const summary of msg.threads) {
    ...
    if (current) currentTurnState = summary.turn_state;
  ...
  renderPersistenceRecovery(state.threadId, currentTurnState);
  renderAllRuntimeSummaryIndicators();
  renderSubagentsButton();
}
...
function validatedBootstrapPayload(staged) {
  ...
  if (!payload.history || !["full_page", "delta", "cursor_reset"].includes(payload.history.kind) ||
      !Array.isArray(payload.history.turns)) {
...
function commitThreadBootstrap(msg) {
  ...
  for (const event of payload.ordered_suffix) applyThreadEventPayload(event.event, staged.threadId);
...
function subscribeCurrentThread(delayMs) {
  const subscribe = () => {
    ...
    if (state.newestPersistedTurnId) request.since = state.newestPersistedTurnId;
    send(request);
  };
...
function handleResyncRequired(msg) {
  ...
  if (!stagedMatches && !activeMatches) return;
...
function handleThreadEvent(msg) {
  ...
  applyThreadEventPayload(msg.event, threadId);
...
function renderHistoryPage(msg) {
  if (!Object.prototype.hasOwnProperty.call(msg, "before")) throw new Error("history_page is missing before");
  const pending = state.paginationRequest;
  const before = msg.before === null ? null : String(msg.before);
  if (!pending || pending.threadId !== String(msg.thread_id || "") || pending.before !== before) return;
  state.paginationRequest = null;
  state.loadingHistory = false;
...
function applyBootstrapHistory(history) {
  ...
  if (history.kind === "full_page" || history.kind === "cursor_reset") {
    ...
    state.newestPersistedTurnId = turns.length ? turns[turns.length - 1].id : null;
  ...
  for (const turn of turns) renderPersistedTurn(turn);
  if (turns.length) {
    state.newestPersistedTurnId = turns[turns.length - 1].id;
...
function maybeAutoFillHistory() {
  ...
  state.paginationRequest = { threadId:String(state.threadId), before };
  if (!send({ type:"load_history", thread_id: state.threadId, before, limit: HISTORY_FILL_BATCH })) {
    state.loadingHistory = false;
    state.paginationRequest = null;
  }
}
...
function onTranscriptScroll() {
  ...
  if (t.scrollTop < 80 && state.hasMoreHistory && !state.loadingHistory && state.oldestTurnId && state.threadId) {
    ...
    state.loadingHistory = true;
    state.paginationRequest = { threadId:String(state.threadId), before };
    if (!send({ type:"load_history", thread_id: state.threadId, before })) {
      state.loadingHistory = false;
      state.paginationRequest = null;
    }
  }
...
function renderPersistenceRecovery(threadId, turnState) {
...
function applyFinalRuntime(threadId, runtime) {
  ...
  renderPersistenceRecovery(threadId, runtime.turn_state);
}
