...
- `~/.claude/settings.json` is the **user scope of the settings schema** — the same shape as the
  ...
  included. **Verified:** a user-scope settings file granting an outside directory made a `Write`
  beyond the workspace proceed with no ask under `--setting-sources user`, and the same file was
  ignored — one ask — under `--setting-sources ""`. (Tested with `CLAUDE_CONFIG_DIR` pointed at a
  throwaway directory, which is also the clean way to sandbox a child from the real config.)
- `~/.claude.json` is **account and machine state, not configuration**: a freshly created one held
  ...
  notification state, and a per-project map. It contained no `permissions` block and no
  `additionalDirectories`. It is where the OAuth session lives, which is why credentials survive
  `--setting-sources ""` untouched. (Observed on a file generated by these headless runs; a long-lived
  one accumulates more per-project entries, so treat this as its shape rather than an exhaustive
  schema.)

The practical consequence for Giskard: **the permission surface is settings-schema state, so under
`--setting-sources ""` no file on the machine contributes to it** — not the user's, not the project's —
while authentication is unaffected because it is not settings-schema state at all.
