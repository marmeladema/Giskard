...
- The active project surface may expose a compact read-only Git status row near the composer:
  current branch or detached head, dirty counts, changed files, and staged/unstaged diffs for
  workspace-relative paths. Staging, committing, branch creation/switching, and hunk mutation stay
  out of scope.
