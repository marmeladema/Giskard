...
**Changelog (1.66 → 1.67), runtime and bootstrap reconciliation:**
...
- **RB7:** Turn-less context-window restoration is deliberately deferred. When added, it must use
  the existing metadata authority and ordinary `ThreadMetadata` publication. Durable late-command
...
- `ThreadBootstrap { thread_id, subscription_generation, frame }` is one staged transaction.
  ...
  payload.
- `HistoryPage { thread_id, before, turns, has_more }` is pagination only. It cannot be interpreted
  as bootstrap history and echoes the requested cursor, including `null`.
...
`PersistenceBlocked` state, the task baseline, and every current request state. The live-turn
...
included in the same transaction. Late command events carry no history coverage and are never
suppressed merely because their turn is already persisted.
...
- **Runtime overview and hidden threads:** the overview contains exact membership for threads with
  an active or persistence-blocked turn or a pending/responding request. An empty replacement clears
