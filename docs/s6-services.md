...
| Registry-side production reads of the five fields: 21 in `registry.rs` (`shared.runtime` ×15 at `:465, :496, :524, :548, :555, :860, :978, :1005, :1040, :1084, :1398, :1418, :1603, :1671, :1710`; `shared.store` ×3 at `:750, :1754, :1847`; `shared.hub` `:1100`; `shared.thread_metadata` `:632`; `shared.ledger` `:1512`) and 4 in `registry/admission.rs` (`shared.store` at `:122, :138, :178, :226`). `driver.rs` reads one, `shared.store` in `begin_link` (`:702-703`, split over two lines and missed by the first cut's single-line grep; revision 2) | grep |
...
| `crates/giskard-server/src/registry/driver.rs` | `:614-631` passes `services`; `:702-703` reads `shared.services.store`; 3 test reads |
