...
commit. The browser applies nothing before a matching-generation commit. Older-history pagination
does not use the WebSocket.

`GET /api/projects/{id}/threads/{thread_id}/history?before={turn_id}&limit={count}` returns older
completed turns as `{ before, turns, has_more }`, oldest first. `before` is required and echoed in
the response; the initial newest page comes from `ThreadBootstrap`. `limit` is optional and
defaults to `[history].page`; the server clamps both requested and configured values to 1–100 turns.
The endpoint is authenticated, verifies that the thread belongs to the project in the path, and is
independent of subscription generations and live-event delivery.
...
`Subscribe.since`, when present, is
`{ newest_turn_id, server_epoch, amendment_sequence }`. The turn id clocks index appends; the
process epoch and sequence clock appended replacement items in older turn payloads. A stale
epoch or sequence produces a cursor-reset bootstrap rather than a compatibility fallback.
...
A failed late-command amendment is orthogonal to turn activity and appears as
`history_recovery` in final runtime and the runtime overview. A newer turn may remain active. The
browser uses `RetryHistoryAmendment { thread_id, recovery_id }` or, after confirmation,
`DiscardHistoryAmendment { thread_id, recovery_id }`. Amendments are queued per thread; later
results cannot persist or publish ahead of a failed predecessor. Discard publishes the visible
completion without durable coverage and retains a reconnect-visible warning that it will not
survive restart.
